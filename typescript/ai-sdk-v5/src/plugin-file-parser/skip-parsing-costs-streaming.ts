/**
 * Example: OpenRouter FileParserPlugin - Skip Parsing Costs with Streaming (AI SDK)
 *
 ...
 * using streamText instead of generateText.
 *
 * Key Points:
 ...
 *
 * To run: bun run typescript/ai-sdk-v5/src/plugin-file-parser/skip-parsing-costs-streaming.ts
 */
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════════════════════╗');
  console.log('║   OpenRouter FileParserPlugin - Skip Parsing Costs Streaming (AI SDK)      ║');
  console.log('╚════════════════════════════════════════════════════════════════════════════╝');
  console.log();
  console.log('This example demonstrates how to reuse annotations to skip PDF re-parsing.');
  console.log('Using streamText instead of generateText.');
  ...
  console.log('STEP 1: Initial request with streaming (PDF will be parsed)');
  console.log('─'.repeat(70));
  ...
  // Extract annotations from providerMetadata (available after stream completes)
  const orMetadata = (await firstResult.providerMetadata)?.openrouter as
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('─'.repeat(70));
  console.log('STEP 2: Follow-up WITH annotations using streaming (parsing SKIPPED)');
  console.log('─'.repeat(70));
  ...
  const withAnnotationsResult = streamText({
    model,
    messages: followUpWithAnnotations,
  });

  process.stdout.write('Response: ');
  for await (const chunk of withAnnotationsResult.textStream) {
    process.stdout.write(chunk);
  }
  console.log();

  const withAnnotationsMetadata = (await withAnnotationsResult.providerMetadata)?.openrouter as
    | { usage?: { cost?: number } }
    | undefined;
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('─'.repeat(70));
  console.log('STEP 3: Follow-up WITHOUT annotations using streaming (PDF re-parsed)');
  console.log('─'.repeat(70));
  ...
  const withoutAnnotationsResult = streamText({
    model,
    messages: followUpWithoutAnnotations,
  });

  process.stdout.write('Response: ');
  for await (const chunk of withoutAnnotationsResult.textStream) {
    process.stdout.write(chunk);
  }
  console.log();

  const withoutAnnotationsMetadata = (await withoutAnnotationsResult.providerMetadata)?.openrouter as
    | { usage?: { cost?: number } }
    | undefined;
